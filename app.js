...
function usersSpeciality(specialty){
    return users.filter(user => user.specialty.toLowerCase() === specialty.toLowerCase());
}
...
app.get("/", (req, res) =>{
    ...
    res.send(`
        <h1>HOME</h1>
        <div>
            <a href="/marketing"><b>MARKETING</b></a>
            <a href="/developers"><b>DEVELOPERS</b></a>
            <a href="/QAs"><b>QAs</b></a>
            <a href="/ventas"><b>VENTAS</b></a>
        </div>
        `);
});

app.get("/marketing", (req, res) =>{
    const usersMarketing = usersSpeciality("marketing");
    res.send(`<h1>MARKETING (${usersMarketing.length} PERSONAS)</h1>
        <div>
            <ul>
                ${usersMarketing.map(user => `<li>${user.name} - ${user.age} años</li>`).join('')};
...
app.get("/developers", (req, res) =>{
    const usersDevelopers = usersSpeciality("developers");

    res.send(`<h1>DEVELOPERS (${usersDevelopers.length} PERSONAS)</h1>
        <div>
            <ul>
                ${usersDevelopers.map(user => `<li>${user.name} - ${user.age} años</li>`).join('')};
...
app.get("/QAs", (req, res) =>{
    const usersQAs = usersSpeciality("QAs");
    res.send(`<h1>QAs (${usersQAs.length} PERSONAS)</h1>
        <div>
            <ul>
                ${usersQAs.map(user => `<li>${user.name} - ${user.age} años</li>`).join('')};
...
app.get("/ventas", (req, res) =>{
    const usersVentas = usersSpeciality("ventas");
    res.send(`<h1>VENTAS (${usersVentas.length} PERSONAS)</h1>
        <div>
            <ul>
                ${usersVentas.map(user => `<li>${user.name} - ${user.age} años</li>`).join('')};
...
app.listen(3000, ()=>{
    console.log("node escuchando puerto 3000")
});